...
| :--- | :--- |
| **`main.cpp`** | **El Punto de Entrada (Orquestador):** Este archivo contiene las funciones `setup()` y `loop()` del ESP32. Se encarga de crear las instancias de todos los gestores (`CronoMgr crono;`, `DisplayMgr display;`, etc.) y de llamar sus métodos de actualización cíclicamente, coordinando la operación de todo el sistema. |
